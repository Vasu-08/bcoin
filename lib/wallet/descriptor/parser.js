  ...
  parsePubkeyInner(desc, keyIndex, context, network) {
    ...
    if (keySplit.length === 1) {
      ...
      if (isHex(str)) {
        ...
        // const pubkey = ring.getPublicKey('hex');
        if (permitUncompressed || isCompressedEncoding(serializedKey)) {
          const options = {
            index,
            ring,
            network
      ...
      if (!HD.isBase58(str, network)) {
        const ring = KeyRing.fromSecret(str, network);
        if (permitUncompressed || isCompressedEncoding(ring.publicKey)) {
          const options = {
            index,
            ring,
            network
  ...
  parseType(desc, keyIndex, context, network) {
    ...
    if (  (context === scriptContext.TOP
       ...
       && (descType('multi', expr) || descType('sortedmulti', expr))
        ...
        assert(
          m <= n,
          `Threshold greater than number of keys (${m} > ${n})`
        );
    ...
    if (   (context === scriptContext.TOP
      ...
      const pubkeyprovider = this.parsePubkey(
        desc,
        keyIndex,
        scriptContext.P2WPKH,
        network
      );
      keyIndex.index++;
